...
const data = fs_1.default.readFileSync('./data/G2Extend.hera', 'utf8');
...
function runTransition(transition) {
    let i = 1;
    let newSymbolTable = new Map(symbolTable);
    for (let elt of bindingsList === null || bindingsList === void 0 ? void 0 : bindingsList.bindings) {
        const g = (0, ts_graphviz_1.digraph)('G' + i);
...
function generatingReachabilityGraph(rg) {
    let dg = (0, ts_graphviz_1.digraph)('RG');
    let i = 0;
    for (let elt of rg.stateMap.values()) {
        let currentState = 'S0';
        i++;
        let currentOutgoingTransition = existSuccessors(elt);
...
function graphToImagePng(g, imageName) {
    ...
    console.log(dot);
    const render = (0, graphviz_cli_renderer_1.CliRenderer)({ outputFile: "./outPut/" + imageName + ".png", format: "png" });
    -+(async () => {
        try {
            await render(dot);
        }
