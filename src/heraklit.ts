import { attribute, Digraph, digraph, toDot } from 'ts-graphviz';
import fs from "fs"
...
export default class Heraklit {
  lines!: string
  symbolTable: Map<string, Symbol> = new Map();
  dg: Digraph
  data!: any
  constructor(filePath: string, digraphName: string) {
    this.data = fs.readFileSync(filePath, 'utf8')
    this.lines = this.data.toString().replace(/\r\n/g, '\n').split('\n');
    this.dg = digraph(digraphName)
  }

  HeraklitEngine() {
    this.setSymboleTableByReading(this.lines)
    this.addValueToSymbolTable(this.lines)
    this.getValueByKey(this.lines)
    this.graphCreated(this.symbolTable)
    this.readFnAssociation()
    return this.computeAllState(this.symbolTable)
  }
   ...
   * @param lines 
   */
  setSymboleTableByReading(lines: any) {
    for (const line of lines) {
      ...
      if (relName === 'Place') {
        ...
        this.symbolTable.set(name, transition)
      }
    }
  }
   ...
   * @param lines 
   */
  addValueToSymbolTable(lines: any) {
    ...
    for (const line of lines) {
      ...
      if (relName === 'Flow') {
        ...
        const srcSymbol = this.symbolTable.get(srcName) as Symbol
        const tgtSymbol = this.symbolTable.get(trgName) as Symbol
        ...
        const keySymbol = this.symbolTable.get(key) as Transition
        ...
        for (let t of this.symbolTable.keys()) {
          let symbol = this.symbolTable.get(t) as Transition
          if (symbol?._type === 'Transition') {
            symbol as Transition
            if (!symbol?.equations.get(relName) && !this.symbolTable.get(relName)) {
    ...
    if (typeValue) {
      this.symbolTable.set('declaration', typeValue)
    }
  ...
  getValueByKey(lines: any) {
    for (const line of lines) {
      ...
      let valueSymbol = this.symbolTable.get(relName)
      let i = 2
      ...
      if (valueSymbol?._type === 'Place') {
        ...
        this.symbolTable.get(valueSymbol.name)?.value.set(name, val)
      } else {
  ...
  readFnAssociation() {
    for (let line of this.lines) {
      ...
      if (length >= 6 && tokenList[1] === '(' && tokenList[3] === ')' && tokenList[4] === '=') {
        let fn = this.symbolTable.get(relName)
        if (!fn) {
          fn = new Symbol()
          fn.name = relName
          fn._type = 'Function'
          this.symbolTable.set(relName, fn)
        }
  ...
  graphCreated(symbolTable: any) {
    ...
    for (let elt of symbolTable.keys()) {
      ...
      if (value._type === 'Transition') {
        ...
        for (let item of value.inFlows) {
          ...
          const node = this.dg.createEdge([item.value.get('src').name, elt], {
        ...
        for (let item of value.outFlows) {
          ...
          const node = this.dg.createEdge([elt, item.value.get('tgt').name], {
    ...
    this.graphToImagePng(this.dg, 'Gen')
  }
   ...
   * @param startState 
   */
  computeAllState(startState: Map<string, Symbol>) {

    // Create Reachability graph
    let rg: ReachabilityGraph = new ReachabilityGraph()
    let key = this.generatingHeraklitString(startState)
    ...
    while (todoList.length > 0) {
      ...
      todoList.splice(0, 1)
      this.expandOneState(rg, todoList, currentState)
    }
    for (let stateItem of rg.stateMap.values()) {
      this.generatingGraphState(stateItem)
    }
    ...
    this.graphToImagePng(gr, 'reachabilityGraph')
    return this.OperatorExec(rg)
  }
   ...
   * @param state 
   */
  expandOneState(g: ReachabilityGraph, todoList: ReachableState[], state: ReachableState) {
    //for each transition 
    for (let s of state.symbolTable.keys()) {
      ...
      if (eSymbol._type === 'Transition') {
        ...
        for (let flow of trans.outFlows) {
          let varList = flow.list
          transitionBindings.expandOut(varList, this.symbolTable, trans)
        }
        //for each binding
        for (let b of transitionBindings.bindings) {
          this.doOneBinding(g, todoList, state, b, trans)
        }
      }
    }
  }
   ...
   * @returns 
   */
  doOneBinding(g: ReachabilityGraph, todoList: ReachableState[], state: ReachableState, currentBinding: Map<string, string>, transition: Transition) {
    ...
    //is it a new state?
    let newKey = this.generatingHeraklitString(cloneState)
   ...
   * @returns 
   */
  generatingHeraklitString(state: Map<string, Symbol>) {
   ...
   * @param state 
   */
  generatingGraphState(state: ReachableState) {
    let dg = digraph('G')
    ...
    this.graphToImagePng(dg, state.name)
   ...
   * @param imageName 
   */
  graphToImagePng(g: any, imageName: string) {
    const dot = toDot(g);
   ...
   * @returns 
   */
  V1ShoesPredicate(state: ReachableState) {
    for (let elt of state.symbolTable.values()) {
      if (elt._type === 'Place') {
   ...
   * @returns 
   */
  AliceShoesPredicate(state: ReachableState) {
    for (let elt of state.symbolTable.values()) {
      if (elt._type === 'Place') {
        console.log(elt._type)
   ...
   * @param rg 
   */
  OperatorExec(rg: ReachabilityGraph) {
    // init state
    ...
    if (ef.find(systemState, this.V1ShoesPredicate)) {
    ...
    if (ef.find(systemState, this.AliceShoesPredicate)) {
      console.log("FOUND PATH:")
      let path = ""
      for (const state of ef.example) {
        path += state.name + " -> "
