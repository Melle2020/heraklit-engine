...
const g = digraph('G');
const data = fs.readFileSync('./data/logicLanguage2.logic', 'utf8')
...
for (const line of lines) {
    ...
    if (relName === 'Flow') {
         if(words.length===3){
            ...
            symbolTable.get(name).values.push(name)
         }else
         if(words.length===4){
            symbolTable.set(name, {
                _id: name,
        ...
        console.log("test"+ JSON.stringify(symbolTable.get(name)))
    }
...
for (const line of lines) {
    ...
    if (relName === 'Flow') {
        ...
        console.log("value"+JSON.stringify(srcName))
